...
public class GenerateWorld : MonoBehaviour
{
    ...
    [SerializeField]
    private List<Vector3> spawnvalues;
    ...
    public void CreateShape()
    {
        ...
        for (int index = 0, i = 0; i <= xSize; i++)
        {
            for (int j = 0; j <= zSize; j++)
            {
                ...
                float x = j * spacedout + Random.Range(0, 10);
                float z = i * spacedout + Random.Range(0, 10);
                spawnvalues.Add(new Vector3(x, perlinNoiseY, z));
                verticies[index] = new Vector3(x, perlinNoiseY, z);
        ...
        for (int i = 0; i < spawnvalues.Count; i++) {
            if (spawnvalues[i].y > max) {
                max = spawnvalues[i].y;
                position = spawnvalues[i];        
            }
        }

        FindObjectOfType<Spawner>().setSpawn(position);
